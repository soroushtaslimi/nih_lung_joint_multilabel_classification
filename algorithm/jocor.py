...
class JoCoR:
    ...
    def evaluate_model(self, test_loader, model):
        ...
        # all_outputs = torch.empty(self.num_test_samples, self.num_classes, dtype=torch.float, device=self.device)
        # all_labels = torch.empty(self.num_test_samples, self.num_classes, dtype=torch.int8, device=self.device)
